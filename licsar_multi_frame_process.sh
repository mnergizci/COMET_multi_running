#!/bin/bash
## Script to process multiple LiCSAR frames with optional date range and terminal mode
# 01/04/2024 Muhammet Nergizci, COMET, University of Leeds


# Function to display usage
usage() {
    echo "Usage: $0 <frames_file> [-s start_date] [-e end_date] [-T]"
    exit 1
}
...
##Create LiCSAR_log directory if doesn't exist
# Create LiCSAR_log directory inside BATCH_CACHE_DIR if it doesn't exist
log_dir="${BATCH_CACHE_DIR}/LiCSAR_log"
mkdir -p "$log_dir"
...
while read -r i; do
    ...
    if [ "$terminal_mode" = true ]; then
        ...
        # else
        #     if [ "$terminal_mode" = true ]; then
        #         run_command="batch3; licsar_make_frame.sh -T -f '$i' 0 1 '$start_date' '$end_date'"
        #     else
        #         run_command="batch3; licsar_make_frame.sh  -f '$i' 0 1 '$start_date' '$end_date'"
    # fi

    tmux new-session -d -s "$session_name" \
    "$run_command >> '${log_dir}/${i}_licsar_out.log' 2>> '${log_dir}/${i}_licsar_err.log' && echo 'Job for $i completed; bash'"
done < "$frames_file"
